...
with unit19 as (
    ...
    select
        ...
        max(e.id) filter (where e.title ilike '%Данък придобиване%' or e.title ilike '%3.09%') as transfer_tax_expense_id,
        max(e.id) filter (where e.title ilike '%Broker%' or e.title ilike '%брокер%') as broker_fee_expense_id,
        max(e.id) filter (where e.title ilike '%Нотариални такси%' or e.title ilike '%notary%') as notary_fee_expense_id,
        max(e.id) filter (where e.title ilike '%кадастър%' or e.title ilike '%cadastre%') as cadastre_fee_expense_id,
        max(e.id) filter (where e.title ilike '%E9%' or e.title ilike '%е9%') as e9_fee_expense_id
