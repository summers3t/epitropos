            ...
            {open ? (
                ...
                >
                    <div className="mb-3 flex items-center justify-between gap-2">
                        ...
                        <div className="text-[13px] font-semibold text-[#0b1623]">{formatMonthTitle(visibleMonth)}</div>
                        <button
                            type="button"
                            onClick={() => setVisibleMonth(new Date(visibleMonth.getFullYear(), visibleMonth.getMonth() + 1, 1))}
                            className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#ccd9e8] bg-white/[0.70] text-[#607993] transition hover:bg-white"
                    ...
                    <div className="mb-1 grid grid-cols-7 gap-1">
                        {weekDayLabels.map((label) => (
                            <div key={label} className="px-1 py-1 text-center text-[10px] font-semibold uppercase tracking-[0.08em] text-[#7a90a8]">
                                {label}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        {days.map((day) => {
                            ...
                            const inMonth = day >= startOfMonth(visibleMonth) && day <= endOfMonth(visibleMonth);
