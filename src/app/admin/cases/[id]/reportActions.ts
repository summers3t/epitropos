...
export async function publishReport(reportId: string, formData: FormData) {
  ...
  if (
    caseRow &&
    (!caseRow.decision_status || caseRow.decision_status === "pending")
  ) {
    redirect(
      `/admin/cases/${report.case_id}?reportError=${encodeURIComponent(
  ...
  const { data: storageObject, error: storageObjectError } =
    await supabase.storage
      .from("reports")
      .list(`case-${report.case_id}/report-${report.id}`, {
        limit: 100,
      });

  if (storageObjectError) {
    throw new Error(storageObjectError.message);
  }

  const fileExists = (storageObject ?? []).some(
    (item) =>
      `${`case-${report.case_id}/report-${report.id}`}/${item.name}` ===
      storagePath,
  );
  ...
  if (!fileExists) {
    redirect(
      `/admin/cases/${report.case_id}?reportError=${encodeURIComponent(
        "Uploaded report file is missing from storage. Re-upload the PDF before publishing.",
      )}`,
    );
  }
