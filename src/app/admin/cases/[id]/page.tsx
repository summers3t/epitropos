...
import { deleteCaseAdmin, deleteDraftReport } from "./adminCleanupActions";
...
function getReportSectionHelp({
  reportCount,
  publishedReportCount,
  downloadableReportCount,
}: {
  reportCount: number;
  publishedReportCount: number;
  downloadableReportCount: number;
}) {
  if (
    publishedReportCount > 0 &&
    downloadableReportCount < publishedReportCount
  ) {
    return "At least one published report is missing its attached file. Fix report availability before delivery or closure work.";
  }

  if (publishedReportCount > 0) {
    return "Published deliverables are available. You can review, unpublish, or maintain the written client output for this case.";
  }
...
export default async function AdminCaseDetailPage({
  ...
  async function updateCaseStatus(formData: FormData) {
    ...
    if (newStatus === "delivered") {
      const [{ data: publishedReports, error: publishedReportsError }] =
        await Promise.all([
          supabase
            .from("reports")
      ...
      if (publishedReportsError) {
        throw new Error(publishedReportsError.message);
      }

      const hasPublishedReport = (publishedReports?.length ?? 0) > 0;
      const hasDownloadablePublishedReport = (publishedReports ?? []).some(
        (report) =>
      ...
      if (!hasDownloadablePublishedReport) {
        redirect(
          `/admin/cases/${caseId}?caseStatusError=${encodeURIComponent(
            "Case cannot be delivered without at least one published report file.",
          )}`,
        );
      }

      if (
        !currentCase.decision_status ||
        currentCase.decision_status === "pending"
      ) {
        redirect(
          `/admin/cases/${caseId}?caseStatusError=${encodeURIComponent(
  ...
  async function createCaseProperty(formData: FormData) {
    ...
    const [
      { data: lastProperty, error: lastPropertyError },
      { data: primaryProperty, error: primaryPropertyError },
    ] = await Promise.all([
      supabase
  ...
  const publishedReportCount = (reports ?? []).filter(
    (report) => report.published,
  ).length;
  const downloadableReportCount = (reports ?? []).filter(
    (report) =>
      report.published &&
      typeof report.storage_path === "string" &&
      report.storage_path.trim().length > 0,
    ...
    <section className="space-y-8">
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-white/70">
          ...
          <span>Created {formatAdminDateTime(caseItem.created_at)}</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="space-y-2">
              <p className="text-[11px] text-white/45">
                Flow: Active → Analysis → Delivered → Closed. Delivery requires
                a final conclusion and at least one published report.
              </p>
      ...
      <details className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-6">
          <div>
            <h2 className="text-lg font-semibold text-white">Case Context</h2>
        ...
        {caseItem.status !== "delivered" && caseItem.status !== "closed" ? (
          ...
          >
            <summary className="cursor-pointer list-none p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="font-semibold text-white">Add Property</div>
        ...
        <section className="space-y-6 rounded-3xl border border-emerald-400/20 bg-white/5 p-6 backdrop-blur">
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-white/55">
                Define the final client-facing conclusion that appears in the
                client portal.
              </p>
            ...
            <form action={updateCaseDecision} className="space-y-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-black/10 p-4">
                ...
                <div className="mt-2 space-y-1 text-[11px] leading-5 text-white/45">
                  ...
                  <p data-decision-help="watchlist" className="hidden">
                    Promising case, but not ready for a final client conclusion
                    yet.
                  </p>
                  <p data-decision-help="recommended" className="hidden">
                    Final client conclusion with one recommended property.
                  </p>
                  <p data-decision-help="rejected_all" className="hidden">
              ...
              >
                ...
                <p className="mt-2 text-[11px] leading-5 text-white/45">
                  Optional. Leave empty unless one property should be marked as
                  the lead watchlist candidate.
                </p>
              </div>
              ...
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-2">
                    ...
                    {report.published && !report.storage_path ? (
                      <div className="rounded-md border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                        This published report is missing its attached file.
                        Re-upload the PDF or unpublish until the file is
                        restored.
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {report.storage_path ? (
                      ...
                      <span className="rounded-md border border-amber-400/30 bg-amber-500/10 px-4 py-2 text-xs text-amber-100">
                  ...
                  caseItem.decision_summary ? (
                    <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/5 p-4">
                      ...
                      <p className="mt-2 text-[11px] text-white/45">
                        Keep the executive summary aligned with the
                        client-facing case conclusion.
                      </p>
