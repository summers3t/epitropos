...
export default async function AdminScreeningDetailPage({ params }: PageProps) {
  ...
  const { data: request, error } = await supabase
    ...
    .select(
      "id, user_id, created_at, updated_at, status, name, email, phone, budget_range, budget_min, budget_max, currency, financing_type, goal, risk_tolerance, preferred_markets, decision_timeline, property_identified, listing_url, plan_interest, notes",
    )
    .eq("id", id)
    .maybeSingle();
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="space-y-2">
            <div>
              ...
              <p className="text-sm text-white/80">{request.name || "—"}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              {request.status === "accepted" && sentOffer ? (
                <div className="pt-2 text-xs text-white/55">
                  A sent offer already exists. Sending an offer should move this
                  screening to Offer sent.
                </div>
              ) : null}
